...
namespace Hange.Utility
{
    ...
    public class HTemplate
    {
        ...
        /// </summary>
        private const string TemplateTag = "<ht:template\\s+([0-9a-z]+)>";

        /// <summary>
        /// 引用标签，形如&lt;ht:include header&gt;
        /// </summary>
        private const string IncludeTag = "<ht:include\\s+([0-9a-z]+)>";
        ...
        private const string CDataTag = "<ht:#([0-9a-z]+)>([\\w\\W]+?)</ht:#\\1>";

        /// <summary>
        /// 属性值标签
        /// </summary>
        private const string AttrTag = "@([0-9a-z-_]+)";
            ...
            foreach (Match data in datas)
            {
                ...
                if(type.IsGenericType)
                {
                    ...
                    if (list != null)
                    {
                        ...
                        {
                            html += BindAttr(tmpHtml, item);
                        }
                    }
                }
                else
                {
                    html += BindAttr(tmpHtml, t);
                }
        ...
        /// <returns></returns>
        private static string BindAttr(string html, object obj)
        {
                        ...
                        else
                        {
                            ...
                            if (cType.IsGenericType)
                            {
                                ...
                                if (list != null)
                                {
                                    foreach (var item in list)
                                    {
                                        cHtml += BindAttr(tmp, item);
                                    }
                                }
                            }
                            else
                            {
                                cHtml += BindAttr(tmp, val);
                            }
